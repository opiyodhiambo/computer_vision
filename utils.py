...
def plot_sample(x, y, vae):
    ...
    _, _, _, recon = vae(
        x
    )  # Calls the vae function to reconstruct the output while ignoring all other outputs (_, _, _, _, _, _, _, _, _)
...
class LossHistory:
    ...
    ):  # No smoothing applied in the data. Each loss appended to the list without any smoothing calculations
...
class PeriodicPlotter:
    ...
    def plot(self, data):  # Plots the data
        if (
            time.time() - self.tic > self.sec
        ):  # Checking if the last time interval has passed since the last plot
            ...
            if self.scale is None:
                ...
                raise ValueError(
                    f"{self.scale} is not recognized. Please use a recognized one!)"
                )
            ...
            ipythondisplay.clear_output(
                wait=True
            )  # clears the output displayed in the IPython environment, ensuring that only the current plot is visible
            ipythondisplay.display(plt.gcf())  # displays the current plot

    def create_grid_of_images(
        self, xs, size=(5, 5)
    ):  # Combines a list of images into a single grid. xs: List of images. size: desired size of the grid
        grid = (
            []
        ...
        for _ in range(size[0]):  # Iterating over the desired number of rows
            ...
            row = np.hstack(
                row
            )  # Horizontally stack the images creating a single row of images
            grid.append(row)  # Append the single row to the grid list
        grid = np.vstack(
            grid
        )  # vertically stack the riws in the grid list into a single grid
        return grid
